...
def estimate_telomeric_repeat_length(seq):
    ...
    tel_bp = sum(end - start for start, end in valid_blocks)
    tel_start = min(start for start, end in valid_blocks)
    tel_end = max(end for start, end in valid_blocks)
...
def main():
    parser = argparse.ArgumentParser(
        description="Estimate telomeric repeat length from ONT FASTQ reads using fixed criteria."
    )
